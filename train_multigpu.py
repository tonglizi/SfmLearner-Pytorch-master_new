...
def main():
    ...
    model1 = models.DispNetS()
    if torch.cuda.device_count() > 1:  # 查看当前电脑的可用的gpu的数量，若gpu数量>1,就多gpu训练
        model1 = torch.nn.DataParallel(model1)  # 多gpu训练,自动选择gpu

    disp_net = model1.to(device)
    output_exp = args.mask_loss_weight > 0
    if not output_exp:
        print("=> no mask loss, PoseExpnet will only output pose")
    model2 = models.PoseExpNet(nb_ref_imgs=args.sequence_length - 1, output_exp=args.mask_loss_weight > 0)
    if torch.cuda.device_count() > 1:  # 查看当前电脑的可用的gpu的数量，若gpu数量>1,就多gpu训练
        model2 = torch.nn.DataParallel(model2)  # 多gpu训练,自动选择gpu
    pose_exp_net = model2.to(device)
    ...
    if args.pretrained_exp_pose:
        ...
        pose_exp_net.module.init_weights()

...
from models import PoseExpNetScript
...
parser.add_argument("--pack-model",default=False,help="Pack the model to be used in C++")
...
def main():
    ...
    for j, sample in enumerate(tqdm(framework)):
        ...
        ref_imgs = torch.cat(ref_imgs, 1)
        tgt_img_ref_imgs=torch.cat([tgt_img,ref_imgs],1)
        _, poses = traced_script_module(tgt_img_ref_imgs)

        # if untraced:
        #     #traced_script_module = torch.jit.trace(traced_script_module,tgt_img_ref_imgs)
        #     traced_script_module.save("./"+args.pretrained_posenet+"_toScript.pt")
        #     untraced=False
